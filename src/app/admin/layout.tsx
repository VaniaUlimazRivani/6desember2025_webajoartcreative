...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  // Protect client-side route: replace so user can't go back to protected page
  useEffect(() => {
    if (!mounted) return;
    const loggedIn = typeof window !== 'undefined' ? localStorage.getItem('isLoggedIn') : null;
    if (!loggedIn) {
      // replace instead of push to avoid back navigation to protected pages
      router.replace('/login');
    }
  }, [mounted, router]);

  // Close mobile sidebar when pathname changes (user navigated)
  useEffect(() => {
  ...
  // Determine current menu by startsWith so nested routes resolve to parent
  const currentMenu = menuItems.find((i) => pathname.startsWith(i.href));
  const currentLabel = currentMenu?.label ?? 'Dashboard';

  if (!mounted) {
    return (
      <div className="h-screen flex items-center justify-center text-[#4A403A]">
        Memuat panel admin...
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F7F2] text-[#4A403A] font-sans">
      ...
      {/* Overlay background saat mobile sidebar aktif */}
      {mobileSidebar && (
        <div
      ...
      >
        ...
        <div className="p-6 border-t border-white/10">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 text-red-300 hover:bg-white/10 hover:text-red-200 rounded-xl transition font-medium"
          >
            <LogOut size={20} />
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        {/* HEADER */}
        <header className="bg-white border-b px-8 py-5 flex justify-between items-center sticky top-0 z-20 shadow-sm border-[#E6D5B8]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-12 h-12 rounded-full bg-[#FDFCF5] border-2 border-[#E6D5B8] text-[#C87941] flex items-center justify-center shadow-sm">
              <User size={24} />
            </div>
          </div>
        </header>
